...
class HttpClient extends BaseService {
    ...
    protected static function curl($url, $param, $method = 'post')
    {
        ...
        $log = [
            date('Y-m-d H:i:s')." [ time:{$calculate_time_span} ] url: {$url} method: {$method} data: ".json_encode($param)." result: {$info} errorno: {$_errno} error: {$_error} ",FILE_APPEND
        ];
        $log_obj = new Logger('curl');
        $log_obj->pushHandler(new StreamHandler(storage_path('logs/curl.log')), Logger::INFO);
        $log_obj->info('curl-log', $log);



        if( $_error ){
            return self::err( $_error );
        }
